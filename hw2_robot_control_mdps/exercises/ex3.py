...
def reset_target_position(base_pos: np.ndarray) -> np.ndarray:
    ...
    """
    x = np.random.uniform(0.2, 0.4) + base_pos[0]
    y = np.random.uniform(-0.2, 0.2) + base_pos[1]
    z = np.random.uniform(0.1, 0.4) + base_pos[2]
...
def process_action(action: np.ndarray, jnt_range: np.ndarray) -> np.ndarray:
    ...
    """
    target_qpos = jnt_range[:, 0] + ((1.0 + action)/2) * (jnt_range[:, 1] - jnt_range[:, 0]) 
    return target_qpos
...
def get_obs(qpos: np.ndarray, ee_pos_w: np.ndarray, ee_rot_w: np.ndarray, base_pos_w: np.ndarray, base_rot_w: np.ndarray, target_pos_w: np.ndarray) -> np.ndarray:
    ...
    """
    ee_pos_base = np.dot(base_rot_w.T, (ee_pos_w - base_pos_w))
    ee_rot_base = np.dot(base_rot_w.T, ee_rot_w)
    ee_quat_base = rot_mat_to_quat(ee_rot_base)
    ee_quat_base = quat_normalize(ee_quat_base)
    target_pos_base = np.dot(base_rot_w.T, (target_pos_w - base_pos_w))
